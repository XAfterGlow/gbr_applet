...
module.exports = {
    ...
    DiyFormHandler: function(i, e) {
        var d = e.target.dataset, n = d.type, f = d.field, s = d.datatype, l = i.data.diyform.f_data;
        (t.isArray(l) || "object" != (0, a.default)(l)) && (l = {});
        var m = i.data.diyform.fields;
        ...
        l[f][0] = e.detail.value; else if ("dateend" == n || "timeend" == n) t.isArray(l[f]) || (l[f] = []), 
        l[f][1] = e.detail.value; else if ("checkbox" == n) for (var o in l[f] = {}, e.detail.value) {
            var u = e.detail.value[o];
            l[f][u] = 1;
        } else "radio" == n ? l[f] = e.detail.value : 10 == s ? (t.isEmptyObject(l[f]) && (l[f] = {}), 
        l[f][d.name] = e.detail.value) : l[f] = e.detail.value; else if ("picker" == n) {
            for (var p in l) if (p == f) {
                for (var y in m) if (m[y].diy_type == f) {
                    l[f] = [ e.detail.value, m[y].tp_text[e.detail.value] ];
                    break;
                }
                break;
            }
        } else if ("image" == n) r.upload(function(a) {
            for (var e in l) if (e == f) {
            ...
            l[f].count = l[f].images.length, i.setData({
                "diyform.f_data": l
            });
